...
epilog = """examples:
    app --show-themes
    app --selected-text
    app -bf Arial -tf "Lucida Console" -lf En -lt Uk Hello, world!
    app -t Default -bf "Arial 13" -tf "Lucida Console 13" -lf English -lt Ukrainian --nl2sp Hello, world!
    app --theme Default --button-font "Arial 13" --text-font "'Lucida Console' 13" --language-from English --language-to Ukrainian --nl2sp Hello, world!
    app -t Default -bf "Roboto,arial,sans-serif 12" -tf "Roboto,RobotoDraft,Helvetica,Arial,sans-serif 12" -lf English -lt Ukrainian --nl2sp Hello, world!
"""

parser = argparse.ArgumentParser(prog='app', description="Переклад текстів",
...
parser.add_argument("-lt", "--language-to", metavar="language_to", type=str, nargs="?", default='uk',
                    help="target language, ex. \"ukrainian\" or \"uk\", default: \"uk\"")
...
languages_from = ["auto", "english", "ukrainian"] + \
    sorted(set(languages_dict) - {"auto", "english", "ukrainian"})
languages_to = ["ukrainian", "english"] + \
    sorted(set(languages_dict) - {"auto", "english", "ukrainian"})

sg.theme('Dark Brown 1')
layout = [[sg.InputCombo(values=services, default_value=services[0], readonly=True, text_color='white', background_color='#303030', font=args.button_font, key="-SERVICES-COMBO-"),
           sg.Combo(values=languages_from, default_value=languages_from[0], readonly=False, text_color='white',
                    background_color='#303030', font=args.button_font, key="-LANGUAGES-FROM-COMBO-", expand_x=True),
           sg.Combo(values=languages_to, default_value=languages_to[0], readonly=False, text_color='white', background_color='#303030', font=args.button_font, key="-LANGUAGES-TO-COMBO-", expand_x=True)],
          [sg.Multiline(expand_x=True, expand_y=True, do_not_clear=True, text_color='white',
                        background_color='#303030', font=args.text_font, key="-FROM-")],
          [sg.Button("Перевод", expand_x=True, key="-TRANSLATE-", font=args.button_font),
           sg.Button("🔄", expand_x=False, key="-SWAP-", font=args.button_font, size=(10, None)),
           sg.Checkbox("Замінити абзац на пробіл", text_color='white', background_color='#303030', default=args.nl2sp, font=args.button_font, key="-REPLACE-")],
          [sg.Multiline(expand_x=True, expand_y=True, write_only=False, text_color='white', background_color='#303030', font=args.text_font, key="-TO-")]]

try:
    window = sg.Window("Переводчик", layout, resizable=True, element_padding=2, margins=(2, 2), size=(600, 400),
...
current_language_to = current_language_to if current_language_to in languages_dict.values() else "uk"
...
if args.text is not None and args.text:
    ...
    try:
        if len(current_text) > 5000:
            raise ValueError(f'Забагато символів: {len(current_text)} > 5000')
